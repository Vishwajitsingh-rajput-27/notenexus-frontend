...
export default function AiTutor({ preloadSubject = '' }) {
  ...
  const score = quiz ? quiz.filter((q, i) => quizAnswers[i] === q.answer).length : 0;
    ...
    <div className="max-w-lg mx-auto flex flex-col items-center justify-center min-h-96 pt-8">
      ...
      <div className="glass rounded-2xl p-6 w-full space-y-4">
        <div>
          ...
          <input className="w-full bg-black/20 border border-white/10 rounded-xl px-3 py-2 text-white"
    ...
    <div className="flex flex-col max-w-3xl" style={{ height: 'calc(100vh - 160px)' }}>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pb-4">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          Hi! I'm your <strong>{subject}</strong> tutor. I'll explain clearly and check your understanding. What topic would you like to start with?
        </motion.div>
        <AnimatePresence>
          {history.map((msg, i) => (
